...
class Config:
    def __init__(self):
        if not os.path.isfile(os.path.dirname(os.path.abspath(__file__)) + '/settings.ini'):
            raise Exception("settings.ini does not exist")
        ...
        self.ckan_private = config.getboolean('ckan', 'private', fallback=True)
